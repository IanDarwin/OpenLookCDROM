...
*
* File:         tic_Polyline.c
* RCS:          $Header: /users/npm/src/winterp/src-server/RCS/tic_Polyline.c,v 2.1 1994/06/06 15:41:05 npm Exp $
* Description:  TANGO:POLYLINE_IMAGE_CLASS (subclass of TANGO:IMAGE_CLASS).
* Author:       Niels P. Mayer
* Created:      Sat May 15 21:43:06 1993
* Modified:     Sun Jun  5 14:28:38 1994 (Niels Mayer) npm@indeed
* Language:     C
* Package:      N/A
...
********************************************************************************
*/
static char rcs_identity[] = "@(#)$Header: /users/npm/src/winterp/src-server/RCS/tic_Polyline.c,v 2.1 1994/06/06 15:41:05 npm Exp $";
 ...
 *******************************************************************************/
Boolean Xtango_Get_Polyline_Image_Args(err_string, err_lval, loc_x_float, loc_y_float, lval_color,
				       vertex_seq_len, x_vertex_sequence, y_vertex_sequence,
				       width_float, line_style, arrow_type)
     ...
     double *width_float;	/* If routine returns TRUE, returns line_width */
     double *line_style;	/* If routine returns TRUE, returns line_style */
     int    *arrow_type;	/* If routine returns TRUE, returns arrow_type */
{
  extern LVAL k_NO_ARROW, k_FORW_ARROW, k_BACK_ARROW, k_BOTH_ARROW; /* From tic_Line.c */
  ...
  /* get a sequence of <offset_coord> specifying the vertices of the polyline */
  ...
  /*
   * get <width_float> from argument stack
   */
  if (moreargs()) {
    lval_arg = nextarg();
    if (floatp(lval_arg))
      *width_float = (double) getflonum(lval_arg);
    else {
 ...
 * ==========================================================================
 * TANGO_IMAGE
 * TANGOimage_create(TANGO_IMAGE_TYPE_POLYLINE,lx,ly,vis,col,vert,vx,vy,wid,sty,arr)
 *	double lx,ly;
 *	int vis;
 *	TANGO_COLOR col;
 *	int vert;
 ...
 *	int arr;
 ****************************************************************************/
LVAL Tango_Polyline_Image_Class_Method_ISNEW()
{
  TANGO_IMAGE image;
  ...
  image = TANGOimage_create(TANGO_IMAGE_TYPE_POLYLINE,
			    loc_x_float,
			    loc_y_float,
			    visible_p,
 ...
 ******************************************************************************/
LVAL Tango_Polyline_Image_Class_Method_IMAGE_LOC()
{
  TANGO_IMAGE image;
  LVAL o_image;
  TANGO_PART_TYPE part;
  ...
  if (vertex_idx == -1) {	/* this case occurs only when a TANGO_PART_TYPE arg is given... */
    ...
    if ((vertex_idx < 0) || (vertex_idx > 7))
      xlerror("Invalid tango polyline image vertex index", lval_vertex_idx);
    else if (vertex_idx > (polyline->vertices - 1)) /* remember that polyline->vertices is 1 greater than # of elts in polyline->vertex[] */
      xlerror("Tango polyline image vertex index greater than number of vertices", lval_vertex_idx);
    else if (vertex_idx == 0)
      return (newdcomplex((double) image->loc[0],
			  (double) image->loc[1]));
    else			/* 1 <= vertex_idx <= 7 */
      return (newdcomplex((double) (image->loc[0] + polyline->vertex[vertex_idx - 1][0]),
			  (double) (image->loc[1] + polyline->vertex[vertex_idx - 1][1])));
  }
}
 ...
 * )
 ****************************************************************************/
LVAL Tango_Polyline_Image_Get_Values(image)
     TANGO_IMAGE image;
{
  extern LVAL k_NO_ARROW, k_FORW_ARROW, k_BACK_ARROW, k_BOTH_ARROW; /* From tic_Line.c */
  LVAL lval_location, lval_vertex, lval_color, lval_width, lval_style, lval_arrow, lval_result;
  int i;
  TANGO_POLYLINE_PTR polyline = (TANGO_POLYLINE_PTR) image->object;
  ...
  lval_style = cvflonum((FLOTYPE) polyline->style);
  lval_result = cons(lval_style, lval_result);

  lval_width = cvflonum((FLOTYPE) polyline->width);
  lval_result = cons(lval_width, lval_result);

  lval_color = cvfixnum((FIXTYPE) polyline->color);
  lval_result = cons(lval_color, lval_result);

  for (i = polyline->vertices - 2 ; (i >= 0) ; i--) { /* we're consing vertices, so fetch vertices backwards... */
    lval_vertex = newdcomplex((double) polyline->vertex[i][0], (double) polyline->vertex[i][1]);
...
/*****************************************************************************
 * (send <polyline-image> :storeon)
 *	==> returns list (send TANGO:POLYLINE_IMAGE_CLASS :new <visibility_kwd>
 *				*TANGO_WIDGET*
 *				#C(<location-x> <location-y)
 *				#C(<vert-0-x> <vert-0-y>)
 *					...
 ...
 *				<arrow-kwd>)
 ****************************************************************************/
LVAL Tango_Polyline_Image_Class_Method_STOREON()
{
  ...
  lval_result = Tango_Polyline_Image_Get_Values(image);
...
Tic_Polyline_Init()
{
  LVAL o_TANGO_POLYLINE_IMAGE_CLASS;

  /*--------------- create 'Class' instance 'TANGO:POLYLINE_IMAGE_CLASS' -------------*/
  o_TANGO_POLYLINE_IMAGE_CLASS =
    Tcls_Create_Subclass_Of_TANGOIMAGE_CLASS("TANGO:POLYLINE_IMAGE_CLASS",
					     TANGO_IMAGE_TYPE_POLYLINE);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":ISNEW",
	   FTAB_Tango_Polyline_Image_Class_Method_ISNEW);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":STOREON",
  ...
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":IMAGE_LOC",
  ...
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_COLOR",
   ...
   * thus can't add it to TANGO:IMAGE_CLASS metaclass... */
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_FILL",
  ...
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_RESIZE",
	   FTAB_Tango_Non_Poly_Image_Class_Method_TX_RESIZE);
   */

  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_RESIZE1",
	   FTAB_Tango_Poly_Image_Class_Method_TX_RESIZE1);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_RESIZE2",
	   FTAB_Tango_Poly_Image_Class_Method_TX_RESIZE2);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_RESIZE3",
	   FTAB_Tango_Poly_Image_Class_Method_TX_RESIZE3);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_RESIZE4",
	   FTAB_Tango_Poly_Image_Class_Method_TX_RESIZE4);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_RESIZE5",
	   FTAB_Tango_Poly_Image_Class_Method_TX_RESIZE5);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_RESIZE6",
	   FTAB_Tango_Poly_Image_Class_Method_TX_RESIZE6);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_RESIZE7",
	   FTAB_Tango_Poly_Image_Class_Method_TX_RESIZE7);

  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_GRAB1",
	   FTAB_Tango_Poly_Image_Class_Method_TX_GRAB1);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_GRAB2",
	   FTAB_Tango_Poly_Image_Class_Method_TX_GRAB2);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_GRAB3",
	   FTAB_Tango_Poly_Image_Class_Method_TX_GRAB3);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_GRAB4",
	   FTAB_Tango_Poly_Image_Class_Method_TX_GRAB4);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_GRAB5",
	   FTAB_Tango_Poly_Image_Class_Method_TX_GRAB5);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_GRAB6",
	   FTAB_Tango_Poly_Image_Class_Method_TX_GRAB6);
  xladdmsg(o_TANGO_POLYLINE_IMAGE_CLASS, ":TX_GRAB7",
