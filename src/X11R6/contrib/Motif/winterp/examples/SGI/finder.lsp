...
;
; File:         finder.lsp
; RCS:          $Header: /users/npm/src/winterp/examples/SGI/RCS/finder.lsp,v 2.1 1994/06/06 14:49:27 npm Exp $
; Description:  For SGI Irix 5.X only -- a demo of SG_FINDER_WIDGET_CLASS
...
;		drop pocket widget, etc.
; Author:       Niels P. Mayer
; Created:      Wed May 18 22:54:48 PDT 1994
; Modified:     Sun Jun  5 18:09:04 1994 (Niels Mayer) npm@indeed
; Language:     Lisp
; Package:      N/A
...
(setq toplevel_w
      (send TOP_LEVEL_SHELL_WIDGET_CLASS :new "form1shl"
	    ...
	    :XMN_TITLE		"WINTERP: SGI Finder Test"
	    :XMN_ICON_NAME	"W:SgFinder"
	    ))
...
(setq fi_w
      (send SG_FINDER_WIDGET_CLASS :new :managed "finder" form_w
	    ))

(setq dp_w (send fi_w :get_child :drop_pocket))

(send dp_w :add_callback :SGN_ICON_UPDATE_CALLBACK 
...
(send fi_w :add_history_item (string (gensym "foo")))
(send fi_w :clear_history)
(send fi_w :get_string)
(send fi_w :set_string (string (gensym "you-can-set-this")))
(send fi_w :get_child :drop_pocket)
(send fi_w :get_child :text)
;; returns a WIDGET_CLASS instance since SgZoomBar is not def'd in WINTERP
(send fi_w :get_child :zoom_bar)
;; returns a WIDGET_CLASS instance since SgDynaMenu is not def'd in WINTERP
(send fi_w :get_child :history_menubar)

(send fi_w :set_callback :XMN_ACTIVATE_CALLBACK '(CALLBACK_WIDGET CALLBACK_REASON)
      '(
	(format T "~A: widget=~A name=~S string=~S\n"
		CALLBACK_REASON
		CALLBACK_WIDGET
...
(send fi_w :set_callback :XMN_VALUE_CHANGED_CALLBACK '(CALLBACK_WIDGET CALLBACK_REASON)
      '(
	(format T "~A: widget=~A name=~S string=~S\n"
		CALLBACK_REASON
		CALLBACK_WIDGET
