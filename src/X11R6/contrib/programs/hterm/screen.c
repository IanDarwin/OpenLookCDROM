...
Boolean force;			/* ... leading/trailing spaces */
{
	...
	for (row = toprow; row <= maxrow; y += FontHeight(screen), row++) {
...
#ifdef I18N
	   ind = 2 * (lastind + topline);
	   chars = screen->buf[ind];
	   attrs = screen->buf[ind + 1];
#else
	   ...
	   for (; col <= maxcol; col++) {
#ifdef I18N
		if ((attrs[col] & ~FIRST_CHAR) != flags) {
		   p = chars+lastind;
		   xn = n = col-lastind;
		   if (*p & 0x80) {
		       if (lastind > 0) {
			   if (attrs[lastind-1] & FIRST_CHAR) {
			       x -= FontWidth(screen);
			       p--;
			       n++;
			       xn++;
			    ...
			    if (ind > 0)
				if (*(screen->buf[ind-1]+screen->max_col) & FIRST_CHAR) {
				    newstr[0] = *(screen->buf[ind-2]+screen->max_col);
				    bcopy(chars+lastind, newstr+1, n);
				    p = newstr;
				    n++;
				    if (x > FontWidth(screen))
				       x -= FontWidth(screen);
				    xn++;
				}
		    }
		    if (adjust_euc_string(p, n))
			n--;
		    XmbDrawImageString(screen->display, TextWindow(screen),
				screen->font_set, gc, x, y, p, n);
		    if (flags & BOLD)
			XmbDrawString(screen->display, TextWindow(screen),
				screen->font_set, gc, x + 1, y, p, n);
#else
		if (attrs[col] != flags) {
	   ...
	   if (adjust_euc_string(p, n))
           if (col == screen->max_col + 1) {
	      n--;
           }
           else if (chars[col] & 0x80) {
              if (p == newstr)
                  newstr[n] = chars[col];
              n++;
           }
    ...
    unsigned *flags;
{
	...
	/* small mouse movements.					*/
       ...
       rows = (height + FontHeight(screen) / 2 - border - FontHeight(screen)
               - screen->border) / FontHeight(screen);
#else
...
#ifdef I18N
               screen->fullVwin.height = height - border - FontHeight(screen)
                   - screen->border;
#else
