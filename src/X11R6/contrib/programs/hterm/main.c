...
#ifdef I18N
	offset(xterm_name), XtRString, "hterm"},
#else
...
#ifdef I18N
    "HTerm*SimpleMenu*menuLabel.vertSpace: 100",
    "HTerm*SimpleMenu*HorizontalMargins: 16",
    "HTerm*SimpleMenu*Sme.height: 16",
    "HTerm*SimpleMenu*Cursor: left_ptr",
    "HTerm*mainMenu.Label:  Main Options (no app-defaults)",
    "HTerm*vtMenu.Label:  VT Options (no app-defaults)",
    "HTerm*fontMenu.Label:  VT Fonts (no app-defaults)",
    "HTerm*tekMenu.Label:  Tek Options (no app-defaults)",
#else
...
#ifdef I18N
{"-is",		"*inputStyle",  XrmoptionSepArg,	(caddr_t) NULL},
...
#endif
#ifdef I18N
{ "-is [over|off|root]",   "input style" },
{ "-fs font_list",         "font name list of font set" },
#endif
...
char **argv;
{
	...
	*/
	...
	/* Init the Toolkit. */
#ifdef I18N
	toplevel = XtAppInitialize (&app_con, "HTerm", 
#else
...
#ifdef I18N
	if (strcmp(xterm_name, "-") == 0) xterm_name = "hterm";
#else
 ...
 */
	...
	conn_mask = 0;
	{
		int *fds;
		int fd_count;
		XInternalConnectionNumbers(screen->display, &fds, &fd_count);
		while (--fd_count >= 0) {
			conn_mask |= 1 << fds[fd_count];
			if (fds[fd_count] >= max_plus1)
			    max_plus1 = fds[fd_count] + 1;
		}
		if (fds) XFree(fds);
