...
in_put()
{
	...
	while (select_mask & conn_mask) {
	    for (i = 0; !(select_mask && (1 << i)); i++)
		;
...
#ifdef I18N
	switch (tolower(term->misc.input_style[1])) {
	case 'f':		/* off the spot */
...
set_status_area()
{
    ...
    if (screen->input_style & XIMStatusArea) {
	area.x = 0;
	area.y = screen->fullVwin.fullheight - screen->fullVwin.f_height;
	area.width = 0;
	area.height = screen->fullVwin.f_height;
        attr_list = (XVaNestedList) XVaCreateNestedList(0, 
                XNArea, &area,
                NULL);
	XSetICValues(screen->ic, XNStatusAttributes, attr_list, NULL);
