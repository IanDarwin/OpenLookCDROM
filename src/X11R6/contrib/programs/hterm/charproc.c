...
{XtNfontSet, XtCFontSet, XtRString, sizeof(char *),
	XtOffsetOf(XtermWidgetRec, misc.font_set), XtRString,
	DEFFONTSET},
{XtNinputStyle, XtCInputStyle, XtRString, sizeof(char *),
	XtOffsetOf(XtermWidgetRec, misc.input_style), XtRString,
	DEFINPUTSTYLE},
#else
...
in_put()
{
	...
	temp_mask = select_mask & conn_mask;
	while (temp_mask) {
	    for (i = 0; !(temp_mask & (1 << i)); i++)
		;
	    XProcessInternalConnection(screen->display, i);
 	    temp_mask &= ~(1 << i);
	}
	select_mask &= ~conn_mask;
    }
    bcnt--;
    return(*bptr++);
}
    ...
    unsigned		flags;
{
   ...
   if(screen->cur_row - screen->topline <= screen->max_row) {
	...
	if (*p & 0x80) {
	   if (screen->cur_col > 0) {
	      col = *(screen->buf[2*screen->cur_row]+screen->cur_col-1);
	      attrs = *(screen->buf[2*screen->cur_row+1]+screen->cur_col-1);
	   }
	   else if (screen->cur_row > 0) {
	      col = *(screen->buf[2*screen->cur_row-2]+screen->max_col);
	      attrs = *(screen->buf[2*screen->cur_row-1]+screen->max_col);
	   }
	   else
	      attrs = 0;
	   if (attrs & FIRST_CHAR) {
		 ...
		 cx -= FontWidth(screen);
	      newstr[0] = col;
	      bcopy(str, newstr+1, len);
	      p = newstr;
	      l = len+1;
	   }
	}
	if (adjust_euc_string(p, l))
	   l--;
	XmbDrawImageString(screen->display, TextWindow(screen),
		      screen->font_set, currentGC, cx, cy, p, l);
#else
...
#ifdef I18N
		        XmbDrawString(screen->display, TextWindow(screen),
				      screen->font_set, currentGC, cx+1, cy, p,
				      l);
#else
...
#ifdef I18N
	screen->menu_font_names[fontMenu_fontdefault] = term->misc.font_set;
	if (!LoadNewFont(screen, term->misc.font_set, False, 0)) {
	   fprintf(stderr, "%s: can't open font set\n\t\"%s\"\n",
		   xterm_name, term->misc.font_set);
	   Exit(1);
	}
#else
...
#ifdef I18N
	c = term->misc.input_style[1];
	if (isupper(c)) c = tolower(c);
	switch (c) {
	case 'f':		/* off the spot */
	   screen->input_style = XIMPreeditArea|XIMStatusArea;
	   break;
	case 'o':		/* root window */
	   screen->input_style = XIMPreeditNothing|XIMStatusNothing;
	   break;
	default:		/* over the spot */
	   screen->input_style = XIMPreeditPosition|XIMStatusArea;
	}
	if (!(im = (XIM)XOpenIM(XtDisplay(term), NULL, NULL, NULL))) {
	   fprintf(stderr, "Please run \"is\" first\n");
	   Cleanup(ERROR_VINIT);
	}
	over_spot.x = 0;
	over_spot.y = screen->ascent;
	preedit_list = (XVaNestedList)XVaCreateNestedList(0,
		XNSpotLocation, &over_spot,
		XNForeground, screen->foreground,
	        XNBackground, term->core.background_pixel,
		XNFontSet, screen->font_set,
		NULL);
        status_list = (XVaNestedList) XVaCreateNestedList(0, 
                XNForeground, screen->foreground,
                XNBackground, term->core.background_pixel,
                XNFontSet, screen->font_set,
                NULL);
	screen->ic = (XIC)XCreateIC(im,
				    XNInputStyle, screen->input_style,
				    ...
				    NULL);
	if (im != XIMOfIC(screen->ic)) {
	   fprintf(stderr, "PANIC ic != ic\n");
...
#ifdef I18N
       || curvt->misc.font_set != newvt->misc.font_set) {
       if (curvt->misc.font_set != newvt->misc.font_set)
...
#ifdef I18N
	   if (curvt->misc.font_set != newvt->misc.font_set)
	      newvt->screen.menu_font_names[fontMenu_fontdefault] = curvt->misc.font_set;
#else
	    if(curvt->misc.f_n != newvt->misc.f_n)
...
ShowCursor()
{
	...
	if (screen->cur_row - screen->topline > screen->max_row)
		return;
#ifdef I18N
	y = 2 * (screen->cursor_row = screen->cur_row);
	...
	XmbDrawImageString(screen->display, TextWindow(screen),
			   screen->font_set, currentGC, x, y, (char *)c, n);
#else
...
#ifdef I18N
	if((flags & BOLD)) /* no bold font */
	   XmbDrawString(screen->display, TextWindow(screen),
			 screen->font_set, currentGC, x + 1, y,
			 (char*)c, n);
#else
...
HideCursor()
{
	...
	y = y + screen->ascent;
	XmbDrawImageString(screen->display, TextWindow(screen),
			   screen->font_set, currentGC, x, y, c, n);
#else
	...
	if((flags & BOLD))
	   XmbDrawString(screen->display, TextWindow(screen),
			 screen->font_set, currentGC, x + 1, y, c, n);
#else
    ...
    int *format;
{
	...
	if (len > 1000  ||  strchr(val, '\n'))
	    return;
#ifdef I18N
	if (!LoadNewFont(&term->screen, val, True, fontMenu_fontsel))
#else
...
#ifdef I18N
void SetVTFont(i, doresize, name1)
   int i;
   Bool doresize;
   char *name1;
#else
...
#ifdef I18N
    if (!LoadNewFont(screen, name1, doresize, i)) {
#else
...
#ifdef I18N
int LoadNewFont(screen, nfontname, doresize, fontnum)
#else
    ...
    int fontnum;
{
   ...
   XFontSetExtents *fse;
    ...
    char *tmpname = NULL;
#ifdef I18N
   char **missing_charset_list, *def_string, *fn_list;
   int missing_charset_count;
   XVaNestedList attr_list, list;
#endif
...
#ifdef I18N
   if (!(font_set = XCreateFontSet(screen->display, nfontname,
				   &missing_charset_list,
				   &missing_charset_count,
				   &def_string))) goto bad;
   while (missing_charset_count--) {
...
#endif
#ifdef I18N
   fse = XExtentsOfFontSet(font_set);
   screen->ascent = - fse->max_logical_extent.y;
   screen->descent = fse->max_logical_extent.height + fse->max_logical_extent.y;
#else
    screen->fnt_norm = nfs;
    screen->fnt_bold = bfs;
...
#ifdef I18N
   /* inform input server about the new font */
   if (screen->input_style & XIMStatusArea) {
      attr_list = (XVaNestedList)XVaCreateNestedList(0,
		  XNFontSet, screen->font_set,
		  NULL);
      XSetICValues(screen->ic, XNStatusAttributes, attr_list, NULL);
   }
   if (screen->input_style & XIMPreeditPosition ||
       screen->input_style & XIMPreeditArea) {
      attr_list = (XVaNestedList)XVaCreateNestedList(0,
		   XNFontSet, screen->font_set,
		   NULL);
      XSetICValues(screen->ic, XNPreeditAttributes, attr_list, NULL);
   }
#endif
    return 1;
  ...
  bad:
#ifdef I18N
   if (font_set) XFreeFontSet(screen->display, font_set);
#endif
...
#ifdef I18N
    i = XFontsOfFontSet(screen->font_set, &fs_list, &fn_list);
    screen->fullVwin.f_width = (i == 1 || (fs_list[0]->max_bounds.width
				> fs_list[1]->max_bounds.width/2)
				? fs_list[0]->max_bounds.width
				: fs_list[1]->max_bounds.width/2);
    ...
    TScreen *screen;
{
	...
	xgcv.background = cc;
#ifdef I18N
	XFontsOfFontSet(screen->font_set, &fstruct, &fname);
	xgcv.font = fstruct[0]->fid;
#endif
...
set_pre_edit()
{
   ...
   if (screen->input_style & XIMPreeditPosition) {
      over_spot.x = CursorX(screen, screen->cur_col);
      over_spot.y = CursorY(screen, screen->cur_row) + screen->ascent;
      spot_area.x      = screen->scrollbar;
      spot_area.y      = 0;
      spot_area.height = screen->fullVwin.height;
      spot_area.width  = screen->fullVwin.width - spot_area.x;
      line_space = FontHeight(screen);
      attr_list = (XVaNestedList)XVaCreateNestedList(0,
                    XNSpotLocation, &over_spot,
		    XNArea, &spot_area,
...
set_status_area()
{
    ...
    if (screen->input_style & XIMPreeditArea) { /* off-the-spot */
	preedit_list = (XVaNestedList) XVaCreateNestedList(0,
			XNArea, &pre_area, NULL);
	XGetICValues(screen->ic, XNPreeditAttributes, preedit_list, NULL);
        pre_area.y = screen->fullVwin.fullheight - screen->fullVwin.f_height;
    }
    if (screen->input_style & XIMStatusArea) {
	status_area.x = 0;
	status_area.y = screen->fullVwin.fullheight - screen->fullVwin.f_height;
	status_area.width = 0;
	status_area.height = screen->fullVwin.f_height;
        status_list = (XVaNestedList) XVaCreateNestedList(0, 
                XNArea, &status_area,
                NULL);
    }
    if (screen->input_style & XIMPreeditPosition) { /* over-the-spot */
      pre_area.x      = screen->scrollbar; /* not to overlap */
      pre_area.y      = 0;
      pre_area.height = FontHeight(screen);
      pre_area.width  = screen->fullVwin.width - pre_area.x;
      preedit_list = (XVaNestedList)XVaCreateNestedList(0,
		    XNArea, &pre_area,
		    NULL);
    }
    if (preedit_list && status_list) {
        list = (XVaNestedList) XVaCreateNestedList(0,
                XNPreeditAttributes, preedit_list,
                XNStatusAttributes, status_list,
                NULL);
    ...
    if (list) {
        XSetICValues(screen->ic, XNVaNestedList, list, NULL);
    }
}

reverse_video_ic()
{
    XVaNestedList preedit_list, status_list, list;
    TScreen *screen = &term->screen;
...
adjust_euc_string(str, n)
    char *str;
    int n;			/* length of string */
{
    register char *s;

    if (*(str + n - 1) & 0x80) { /* last char Hangul? */
	/* test if first byte */
	for (s = str + n - 2; *s & 0x80 && s >= str; s--)
	    ;
	if ((str + n - s & 1) == 0)
	    return 1;
    }
