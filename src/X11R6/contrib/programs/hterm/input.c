...
#ifdef I18N
	count = XmbLookupString (screen->ic, event, tmp, STRBUFSIZE,
...
#ifdef I18N
	    count = XmbLookupString (screen->ic, (XKeyEvent *)&ev, tmp,
				STRBUFSIZE, &keysym, &compose_status);
#else
...
#ifdef I18N
	if (keysym == XK_Escape) {	/* really needed ? */
		XmbResetIC(screen->ic);
	}
#endif
	if (IsPFKey(keysym)) {
		reply.a_type = SS3;
