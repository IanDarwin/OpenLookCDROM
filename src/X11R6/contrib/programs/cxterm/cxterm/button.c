/* $Id: button.c,v 3.1 1994/06/06 09:41:36 ygz Exp $ */
/***********************************************************************
...
/* 0xf8 0xf9 0xfa 0xfb 0xfc 0xfd 0xfe 0xff */
   161, 161, 161, 161, 161, 161, 161, 255};
#else  /* HANZI */
/*   -    i   c/    L   ox   Y-    |   So */
...
#ifdef HANZI
#define RowCol_HZ(r,c)			\
		(((c) <= screen->max_col) &&	\
		 ((screen->buf[2 * ((r)+screen->topline) + 1][c]) & CHARHANZI))
#define RowCol_2ndByteHZ(r,c)		\
		(((c) <= screen->max_col) &&	\
		 Is2ndByteHZ(screen->buf[2 * ((r)+screen->topline) + 1][c]))

	/* startSCol, endSCol should always be at the beginning of a HZ */
	if (RowCol_2ndByteHZ(startSRow,startSCol))
		startSCol--;
	if (RowCol_2ndByteHZ(endSRow,endSCol))
		endSCol--;
		...
		case SELECTWORD :
			if (startSCol > (LastTextCol(startSRow) + 1)) {
				...
				do {
					...
					--ptr;
#ifdef HANZI
					if (RowCol_HZ(startSRow,startSCol)) {
						--startSCol;
						--ptr;
					}
#endif /* HANZI */
