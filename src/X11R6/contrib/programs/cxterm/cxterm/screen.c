/* $Id: screen.c,v 3.0 1994/06/04 07:36:02 ygz Exp $ */
/***********************************************************************
...
register int length;		/* length of string */
{
	...
	if (flags & CHARHANZI)
		while(length-- > 1) {
			*attrs = flags;	
			*attrs++ |= FIRSTBYTE;		/* 1st byte of HZ */
			length--;
			*attrs = flags;	
			*attrs++ &= ~(FIRSTBYTE);	/* 2nd byte of HZ */
		}
	else
#endif /* HANZI */
	while(length-- > 0)
	   ...
	   if (flags & CHARHANZI) {
	     if ( (!hilite && (flags & INVERSE) != 0) ||
		  (hilite && (flags & INVERSE) == 0) )
	       if (flags & BOLD) gc = screen->hz_reverseboldGC;
	       else gc = screen->hz_reverseGC;
	     else 
	       if (flags & BOLD) gc = screen->hz_normalboldGC;
...
#ifdef HANZI
		if (Is1stByteHZ(attrs[col])) {
			if(chars[col] == 0)
				chars[col] = ' ';
			col++;	/* skip the 2nd byte HZ */
		}
#endif /* HANZI */
		if(chars[col] == 0)
			chars[col] = ' ';
...
HZ_XDrawImString16(display, d, gc, fgcs, x, y, string, length)
     Display *display;
     Drawable d;
     GC gc;
     int fgcs;
...
HZ_XDrawString16(display, d, gc, fgcs, x, y, string, length)
     Display *display;
     Drawable d;
     GC gc;
     int fgcs;
