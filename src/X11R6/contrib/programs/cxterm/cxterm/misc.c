/* $Id: misc.c,v 3.0 1994/06/04 07:36:02 ygz Exp $ */
/***********************************************************************
	...
	*cp = 0;
	...
	case 160:	/* set HANZI input method directory */
		if((cp = malloc((unsigned)strlen(buf) + 1)) == NULL)
			break;
		strcpy(cp, buf);
		if(term->misc.it_dirs)
			free(term->misc.it_dirs);
		term->misc.it_dirs = cp;
		break;

	case 161:	/* set HANZI input method */
		SetHZMode (&term->screen, term->misc.it_dirs, buf, True);
		break;
#endif /* HANZI */
	/*
