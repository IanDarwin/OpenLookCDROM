/* $Id: util.c,v 1.3 1992/08/22 21:20:02 ygz Exp $ */
/***********************************************************************
    ...
    register int n;
{
	...
	if(screen->cur_row - screen->topline <= screen->max_row) {
		...
		if(screen->scroll_amt)
			FlushScroll(screen);
#ifdef HANZI
		/* To insert n blanks, n chars from the right margin of
		 * the screen is to be shifted out.  See how many HZ bytes
		 * in these n chars.
		 */
		for (i = 0; i < n; i++)
			if ((screen->buf[2*(screen->cur_row+screen->topline)+1]
					[screen->max_col-i]) & CHARHANZI)
				nHiByte++;
#endif /* HANZI */
...
#ifdef HANZI
	if (nHiByte % 2) {
		/* odd number of HZ bytes are removed.  One more at the end. */
		XFillRectangle(screen->display, TextWindow(screen), 
			screen->reverseGC,
			CursorX (screen, screen->max_col), cy,
			(unsigned) FontWidth(screen),
			(unsigned) FontHeight(screen));
		ScrnDeleteChar (screen->buf, screen->cur_row, screen->max_col,
				1, screen->max_col + 1);
	}
#endif /* HANZI */
}
    ...
    register unsigned int rect_width, rect_height;
{
	...
	if (toprow + nrows - 1 > screen->max_row)
		RefreshCXtermInput (screen);	/* intersects the input area */
#endif /* HANZI */
	if (toprow + nrows - 1 > screen->max_row)
	...
	XtermWidget termw;
{
	...
	tmpGC = screen->hz_normalGC;
	screen->hz_normalGC = screen->hz_reverseGC;
	screen->hz_reverseGC = tmpGC;

	tmpGC = screen->hz_normalboldGC;
	screen->hz_normalboldGC = screen->hz_reverseboldGC;
	screen->hz_reverseboldGC = tmpGC;
#endif /* HANZI */
