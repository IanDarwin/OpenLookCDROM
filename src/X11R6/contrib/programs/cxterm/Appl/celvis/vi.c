...
#ifdef	SIGTSTP
/* ^Z  suspend		*/	{suspend_elvis,	NO_ARGS,	NO_FLAGS},
#else
	...
	int		flags;	/* various flags regarding cursor mvmt */
{
	...
	if (!(flags & NCOL))
	{
		...
		if (i == BLKSIZE - 1)
		{
			...
			{
				new += plen - 1;
				if (IsIdxHZ (markidx(new), ptext) < 0)
				{	/* cursor on 2nd-byte of a HZ */
					new-- ;
				}
			}
	...
	else
	{
...
#endif
			else if (IsHiBitOn (*text))	/* hanzi */
			{
				i += 2;
				text++;	/* plus one more in "for()" stmt */
			}
			else
			{
				i++;
		...
		{
			text--;
			if (IsIdxHZ ((int)(text - ptext), ptext) < 0)
			{	/* cursor on 2nd-byte of a HZ */
				text--;
			}
		}
