...
#endif
		else if (IsHiBitOn (i))	/* hanzi */
		{
			if (idx == 1)
			{	/* curs right at the 2nd-byte of HZ */
				break;	/* col is at the 1st-byte */
			}
			col += 2;
			text++;	/* plus one more at bottom of loop */
			idx--;	/* plus one more at bottom of loop */
		}
		else
		{
			col++;
	...
	register char	*text;	/* the text to draw */
{
	     ...
	     text++)
	{
		...
		{
			col += 2;
			text++;	/* plus one more as part of "for" loop */
		}
		else
		{
			col++;
	...
	if (!has_AM || col < limitcol || (col == limitcol && has_XN))
	{
		qaddch('\r');
		qaddch('\n');
	}
...
int hasattr(lno, text)
	...
	register char	*text;	/* the text of the line, from fetchline */
{
	return	TRUE ; /* always return TRUE for 16-bit editor */
}
#endif
