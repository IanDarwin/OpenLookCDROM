...
#define GET_TIME(aDev)	bba_get_time( ((maxPhysDevice *)((aDev)->devPtr))->bba)

/* Give a audio device, update current time in the audio device. */
#define UPDATE_TIME(aDev)	\
	(aDev)->dsptime = GET_TIME(aDev); \
	(aDev)->time0 += (((((aDev)->dsptime)<<8) - (((aDev)->oldDevTime)<<8))>>8); \
