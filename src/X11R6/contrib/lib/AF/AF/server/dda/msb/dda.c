...
static char dda_c_rcsid[]="$Header: /crl/audio/AF/server/dda/msb/RCS/dda.c,v 1.4 1994/02/01 15:45:33 tml Exp $";
#endif
...
int 			nDev=0;
msbPhysDevice		physDevices[MAXPHYSDEVICES];	
...
InitDevices(int argc, char **argv)
{
  ...
  while(((devName=getNextDevName()) != (char *)NULL)&&(nDev<MAXPHYSDEVICES)){
    ...
    status = msbInit(devName, &physDevices[nDev]); /* init physical device */
...
ddaProcessArgument (int argc, char *argv[], int i)
{
	...
	if(++argind < argc) {
	    ...
	    if (!msbAssertRate(hifi_rate)) return 0;
            return 2;
	} else
