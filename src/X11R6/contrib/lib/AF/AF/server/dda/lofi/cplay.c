...
#include <server/include/acstruct.h>	/* ac and audiodev included. */
 ...
 */
int
codecPlay(ATime ptime, unsigned char *p, int plen, ACPtr ac)
{
     ...
     */
    CODEC_UPDATE_TIME(aDev);		/* Snapshot current device time. */

    if(DELTA(1,aDev->time0, aDev->timeLastUpdated)>MSTOTICKS(MSUPDATE)){
	codecUpdate(aDev);	 /* Ensure updated through guardband. */
	CODEC_UPDATE_TIME(aDev); /* Snapshot current device time, again. */
    }
