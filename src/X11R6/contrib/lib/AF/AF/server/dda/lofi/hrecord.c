...
static char read_c_rcsid[]="$Header: /crl/audio/AF/server/dda/lofi/RCS/hrecord.c,v 1.15 1993/11/15 21:22:08 tml Exp $";
#endif
		...
		AudioDevicePtr aDev)
{
	...
	register int newLeft, newRight;
	register int oldLeft, oldRight;
	ATime origTime;
	HiFiPrivate	*hPtr = (HiFiPrivate *) aDev->privPtr;
	hPtr = (HiFiPrivate *) hPtr->prim->privPtr;
	...
	if (hPtr->teleport) {
          oldLeft = hPtr->leftInput & TELEMASKI;
          oldRight = hPtr->rightInput & TELERINGMASK;

 	  while(nsamples--) {
		origTime += 1;
	  	*dest++ = newLeft = *lp++ >> 16;
	  	*dest++ = newRight = *rp++ >> 16;
		if ( (oldLeft != (newLeft & TELEMASKI))) {
		  /* generate events */
		  if (( newLeft & TELERINGMASK) != (oldLeft & TELERINGMASK)) {
		    TeleEvent(aDev, origTime, HIFI_LEFT, TELE_EVENT_RING, 
			(newLeft & TELERINGMASK) == 0);
		  }
		  if (( newLeft & TELELOOPMASK) != (oldLeft & TELELOOPMASK)) {
		    TeleEvent(aDev, origTime, HIFI_LEFT, TELE_EVENT_LOOP, 
			(newLeft & TELELOOPMASK) == 0);
		  }
		  oldLeft = newLeft & TELEMASKI;
		}
		if ( (oldRight != (newRight & TELERINGMASK))) {
		  /* generate events */
		  if (( newRight & TELERINGMASK) != 
		      (oldRight & TELERINGMASK)) {
		    TeleEvent(aDev, origTime, HIFI_RIGHT, TELE_EVENT_RING, 
			(newRight & TELERINGMASK) == 0);
		  }
		  oldRight = newRight & TELERINGMASK;
		}
		if(lp >= &lbuf[HIFI_HWBUF]) {
          ...
          /* store new values */
          hPtr->leftInput = oldLeft;
          hPtr->rightInput = oldRight;
	}
	else {
 	  while(nsamples--) {
	  	*dest++ = *lp++ >> 16;
...
void hifi_read_back(ATime time, int len, AudioDevicePtr aDev)
{
	...
	hifi_read(time, p, slen, ((HiFiPrivate *)aDev->privPtr)->leftRecBuf,
			((HiFiPrivate *)aDev->privPtr)->rightRecBuf, aDev);
...
/* If the start of the request is in the future, return nothing */
    ...
    aDev->time0 = prim->time0;		/* Keep this audio device time too. */
