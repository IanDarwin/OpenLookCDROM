...
;------------------------------------------------------------------------------
	; $Id: Ajvmain.asm,v 1.5 1993/11/12 17:42:30 tml Exp $
...
;	movep	#$7,x:m_pcddr		; output bits
	movep	#$0,x:m_pcddr		; output bits

;	move	x:portc_bits,a1
;	movep	a1,x:m_pcd		; set the port C bits

	clr	b
	movep	b0,x:<<m_tx		; init SSI
...
;
	global	ssi_int
ssi_int
	movep	x:m_rx,a1		; get receive data
	jset	#0,x:<<m_sr,ssi_rx_b	; which channel?

; Channel 0 (A)
	bclr	#1,x:<<m_crb		; toggle OF1 (SC1)

	move	a1,x:(r2)		; store in buffer

	move	x:(r2+n2),a1		; fetch transmit data
	movep	a1,x:m_tx		; send it
	move	#0,a1
...
ssi_rx_b
	bset	#1,x:<<m_crb
	movep	x:m_rx,a1		; get receive data
	move	a1,x:(r3)		; store in buffer

	move	x:(r3+n3),a1		; fetch transmit data
	movep	a1,x:m_tx		; send it
	move	#0,a1
