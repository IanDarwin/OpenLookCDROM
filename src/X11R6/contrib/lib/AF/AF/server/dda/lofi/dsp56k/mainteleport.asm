...
; 10/07/93      lcs     Add Teleport code
;------------------------------------------------------------------------------
	; $Id: mainteleport.asm,v 1.2 1993/11/12 17:53:08 tml Exp $
...
;
	global	ssi_int
ssi_int
	jset	#m_tfs,x:<<m_sr,ssi_rx_b	; which channel?

; Channel 0 (A)
	bset	#1,x:<<m_crb		; toggle OF1 (SC1)
; OUTPUT
	move	x:(r2+n2),a1		; transmit data, send it
	move	x:dsp_masko,x1		; load mask
	and	x1,a1			; mask off ctl bit positions
	move	x:dsp_lctl,x1
	or	x1,a1			; or in ctl bits
	movep	a1,x:m_tx		; transmit data, send it

	move	#0,a1
	move	a1,x:(r2+n2)		; backfill silence

; INPUT
	movep	x:m_rx,a1		; get receive data to register
	move	a1,x:(r2)		; data to buffer
	move	x:dsp_maski,x1		; load mask
	and	x1,a1			; mask off except ctl
	move	x:dsp_lstat,x1
	move	#0,a0
	cmp	x1,a			; compare with old value
	jeq	ssi_nc_a		; jmp around hport
	move	a1,x:dsp_lstat		; update previous value
	movep	#11,x:m_htx		; trigger host interrupt
ssi_nc_a
; COMMON
...
; OUTPUT
	move	x:(r3+n3),a1		; transmit data, send it
;	move	x:(r2),a1		; transmit data, send it
	move	x:dsp_masko,x1		; load mask
	and	x1,a1			; mask off ctl bit positions
	move	x:dsp_rctl,x1
	or	x1,a1			; or in ctl bits
...
; INPUT
	movep	x:m_rx,a1		; get receive data to register
	move	a1,x:(r3)		; data to buffer
	move	x:dsp_maski,x1		; load mask
	and	x1,a1			; mask off except ctl
	move	x:dsp_rstat,x1
	move	#0,a0
	cmp	x1,a			; compare with old value
	jeq	ssi_nc_b		; jmp around hport
	move	a1,x:dsp_rstat		; update previous value
	movep	#11,x:m_htx		; trigger host interrupt
ssi_nc_b
