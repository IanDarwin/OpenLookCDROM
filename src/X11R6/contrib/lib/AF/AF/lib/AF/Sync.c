...
void AFSync (register AFAudioConn *aud, ABool discard)
{
    aGenericReply rep;
    register aSyncConnectionReq *req;

    aud = aud->connection;
    LockConnection(aud);
    GetReq(SyncConnection, req);
    (void) _AReply (aud, (aReply *)&rep, 0, aTrue);

    if (discard && aud->head) {
       ((_AFQEvent *)aud->tail)->next = _afqfree;
       _afqfree = (_AFQEvent *)aud->head;
