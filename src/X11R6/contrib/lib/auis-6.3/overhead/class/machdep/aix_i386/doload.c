...
#ifndef NORCSID
#define NORCSID
static char rcsid[]="$Header: /afs/cs.cmu.edu/project/atk-dist/auis-6.3/overhead/class/machdep/aix_i386/RCS/doload.c,v 1.8 1992/12/15 20:58:42 rr2b R6tape $";
#endif
...
struct doload_environment *e;
{
    ...
    safe_read(e, (char *)&(e->filehdr), FILHSZ);
    ...
    if (e->filehdr.f_magic != I386MAGIC && e->filehdr.f_magic != I386SVMAGIC)
	doload_punt(e, "this is not an AIX PS/2 COFF file (bad magic number)");

    /* read optional header */

    if (e->filehdr.f_opthdr > 0) {
	safe_read(e, (char *)&(e->aouthdr), e->filehdr.f_opthdr);
	if (e->mode == List || doload_trace > 1) {
	    printf("\nAOUT HEADER\n");
	    printf("  magic = %x\n",  e->aouthdr.magic);
	    printf("  vstamp= %x\n",  e->aouthdr.vstamp);
	...
	safe_read(e, (char *)&(e->scn[s]), SCNHSZ);
	if (e->mode == List || doload_trace > 1) {
	    printf("\nSECTION HEADER %d\n", s);
	    printf("  name  = %s\n",  e->scn[s].s_name);
	    printf("  paddr = %lx\n", e->scn[s].s_paddr);
	...
	if (s > 0)
	    e->segptr[s] = e->segptr[0] + e->scn[s].s_vaddr;
	...
	for (s = 0; s < e->filehdr.f_nscns; s += 1) {
		...
		if (e->scn[s].s_nreloc > 0) {
		    ...
		    safe_read(e, (char *)rp, e->scn[s].s_nreloc * RELSZ);
...
char *format;
{
	...
	if (j >= e->newsymcount) {
	    e->newsymcount += 1;
	    e->newsym = (SYMENT *)safe_realloc(e, (char *)e->newsym, e->newsymcount * SYMESZ);
...
RELOC **rpp;
{
	...
	if (e->mode == List || doload_trace > 1)
	    cp = printdata(e, cp, base, e->segptr[0] + relocationbound(e, rp, base + size));
	relocate(e, e->segptr[0] + rp->r_vaddr, rp);
    }
    if (e->mode == List || doload_trace > 1) {
	cp = printdata(e, cp, base, base + size);
	fflush(stdout);
    }
...
register struct doload_environment *e;
{
	...
	case STYP_DATA:
	case STYP_BSS:
	    fixsegment(e, e->scn[s].s_nreloc, e->segptr[s], e->scn[s].s_size, &rp);
	    break;
	    }
	}
}
