...
# Some versions of ar(1) have name length limits that result in the "o" being truncated.
# There is a hack below that prints out the "o" if it isn't there.  Ar(1) should be fixed.
    for ( i = 0 ; i < epcount ; i++)
	if (want[library[ep[i]] "." definer[ep[i]]] == "Y")
		if(definer[ep[i]] ~ /^*\.o$/)
			printf "%s %s\n", definer[ep[i]], ep[i];
