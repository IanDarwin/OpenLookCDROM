	...
	int target;
{
	...
	if (target == '\n')
		...
		warns("   Skipping to next %c", target);
...
read_declarations ()
{
  ...
  for (;;)
	...
	{
	  warns("unknown character: %c (\\%03o)", c, c);
	  skip_to_char('%');
	}
    }
}
  ...
  for (;;)
	...
	case '"':
	  ...
	  while (c != match)
	    {
	      if (c == EOF)
		fatal("unterminated string at EOF");
	      if (c == '\n')
		{
      ...
      else if (token == IDENTIFIER)
	{
	  ...
	  if (typename)
		...
		symval->type_name = typename;
	      else if (strcmp(typename, symval->type_name) != 0)
...
/* parse what comes after %thong 
 ...
 The user_token_number of the first is STHONG and any user_token_number from
 the declaration is stored in the second.  The two symbols are linked
 via pointers in their token_buddy fields.
...
parse_thong_decl ()
{
	...
	symval->class = STOKEN;
	symval->type_name = typename;
	symval->user_token_number = STHONG;
	symbol = symval;
	...
	symval->thong_buddy = symbol;	
	symbol->thong_buddy = symval;	

 	nsyms--;   /* thong and buddy combined are only one symbol */
}
...
parse_expect_decl()
{
  ...
  if (count <= 0 || count > 10)
	warn("funny integer after %expect");
  ...
  while (brace_flag ? (count > 0) : (c != ';'))
	...
	case '$':
	  ...
	  if (c == '$')
		...
		fprintf(fguard, ".%s", type_name);
	      if(!type_name && typed)
		warns("$%d of '%s' has no declared type.", n, rule->sym->tag);
	      continue;
	    }
	  else
	    warns("$%c is invalid", c);
  ...
  while (t != TWO_PERCENTS && t != ENDFILE)
      ...
      if (t == IDENTIFIER || t == BAR)
	{
	  ...
	  /* If $$ is being set in default way,
		...
		warns("type clash ('%s' '%s') on default action",
...
packsymbols()
{
  ...
  for (bp = firstsymbol; bp; bp = bp->next)
      ...
      if (bp->class == SNTERM)
	{
      ...
      else if (bp->thong_buddy)
	{
		/* this symbol and its thong_buddy are a single thong defn.
		  allocate a tokno, and assign to both
		  check agreement of ->prec and ->assoc fields 
			and make both the same
		*/
		if (bp->value == 0)
			bp->value = bp->thong_buddy->value = tokno++;

		if (bp->prec != bp->thong_buddy->prec) {
			if (bp->prec != 0 && bp->thong_buddy->prec != 0
					&& bp->user_token_number == STHONG)
				warns("conflicting precedences for %s and %s",
					bp->tag, bp->thong_buddy->tag);
			if (bp->prec != 0) bp->thong_buddy->prec = bp->prec;
			else bp->prec = bp->thong_buddy->prec;
		}

		if (bp->assoc != bp->thong_buddy->assoc) {
			if (bp->assoc != 0 && bp->thong_buddy->assoc != 0
					&& bp->user_token_number == STHONG)
				warns("conflicting assoc values for %s and %s",
					bp->tag, bp->thong_buddy->tag);
			if (bp->assoc != 0) bp->thong_buddy->assoc = bp->assoc;
			else bp->assoc = bp->thong_buddy->assoc;
		}

		if (bp->user_token_number == STHONG)
			continue;  /* do not do processing below for STHONGs */
      ...
      for (bp = firstsymbol; bp; bp = bp->next)
        {
          ...
          if (bp->user_token_number == STHONG) continue;  /* thong symbol */
	  if (token_translations[bp->user_token_number] != 2)
	    warns( "tokens %s and %s both assigned number %d",
  ...
  for (bp = firstsymbol; bp; bp = bp->next)
    {
      ...
      if (bp->user_token_number == STHONG) continue;
      if ('\'' == *symbol) continue;	/* skip literal character */
      if (bp == errtoken) continue; 	/* skip error token */
      if ('\"' == *symbol) 
	{
		/* use literal string only if given a symbol via %thong */
		if (bp->thong_buddy)
			symbol = bp->thong_buddy->tag;
		else
			continue;
	}
