static char *hgghview_rcsid = "$Header: /afs/cs.cmu.edu/project/atk-dist/auis-6.3/contrib/atkbook/hgghds/RCS/hgghview.c,v 1.1 1992/10/06 22:16:25 susan R6tape $";
...
void hgghview__SetDataObject(self, h)
struct hgghview *self;
struct hggh *h;
{
    super_SetDataObject(self, h);
    self->h = h;
    buttview_SetDataObject(self->bv, h->b);
}
...
long param;
{
    ...
    if (message_AskForString(self, 50, "Enter new text for button: ", "<BEEP>", Buf, sizeof(Buf)) >= 0) {
	hggh_SetButtonText(self->h, Buf);
	message_DisplayString(self, 10, "Changed the button text as requested.");
    }
}
...
struct classheader *c;
{
    ...
    proc = proctable_DefineProc("hgghview-toggle", ToggleProc, &hgghview_classinfo, NULL, "toggles the two parts of the hgghview.");
    keymap_BindToKey(hgghview_keymap, "!", proc, 0);

    hgghview_menulist = menulist_New();
    menulist_AddToML(hgghview_menulist, "Toggle Me!", proc, NULL, 0);
    proc = proctable_DefineProc("hgghview-change-button", ChangeButton, &hgghview_classinfo, NULL, "Changes the text in the toggling button.");
    menulist_AddToML(hgghview_menulist, "HGGH,Change Button Text", proc, NULL, 0);
...
void hgghview__ReceiveInputFocus(self)
struct hgghview *self;
{
    struct view *v = lpair_GetNth(self->lp, 0);
