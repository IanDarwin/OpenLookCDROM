...
class ams {
    classprocedures:
      Focus(struct view *v);
      InstallInNewWindow(struct view *v, char *programname, char *windowname, int width, int ht, struct view *focusv) returns struct frame *;
      SetWantSnap(int wantsnap);
      GetAMS() returns struct ams *;
      SetCheckpointFrequency(int freq);
      AddCheckpointCaption(struct captions *c);
      AddCheckpointBodies(struct t822view *b);
      AddCheckpointFolder(struct folders *f);
      AddCheckpointSendmessage(struct sendmessage *s);
      RemoveCheckpointCaption(struct captions *c);
      RemoveCheckpointBodies(struct t822view *b);
      RemoveCheckpointFolder(struct folders *f);
      RemoveCheckpointSendmessage(struct sendmessage *s);
      PlanFolderPrefetch(struct folders *f);
      TimerInit();
      WaitCursor(boolean IsWait);
      SubscriptionChangeHook(char *name, char * nick, int status, struct messages *mess);
      DirectoryChangeHook(char *adddir, char *deldir, struct messages *mess);
      InitializeClassList() returns int;
      ResetClassList();
      GetMailPath() returns char *;
      GetFirstMailClass() returns char *;
      TryDelayedUpdates() returns int;
      CacheDelayedUpdate(char *fullname, char *updatedate);
