 ...
 */
/*	$Header: /home/sau/mgr/nsrc/RCS/put_window.c,v 1.3 91/03/01 11:05:57 sau Exp Locker: sau $
	$Source: /home/sau/mgr/nsrc/RCS/put_window.c,v $
*/
static char	RCSid_[] = "$Source: /home/sau/mgr/nsrc/RCS/put_window.c,v $$Revision: 1.3 $";

/* Very simple terminal emulator (this is an old comment) */
...
register char *buff;					/* ptr to update text */
int buff_count;						/* # of characters */
   ...
   if (W(flags)&W_ACTIVE) {
      ...
      window = bit_create(W(save),SUM_BDR,SUM_BDR,
                          BIT_WIDE(W(window)),BIT_HIGH(W(window)));
      sub_window++;
      }

   if (window==(BITMAP *) 0) {		/* never! */
...
/* standend also sets character attributes */
         ...
         case E_FONT:  			/* pick a new font */
               ...
               if (cnt > 0) {
                  W(esc)[TEXT_COUNT] = 0;
                  if (W(esc)[cnt]>0 && (W(snarf)=malloc(W(esc)[cnt]+1)) != (char *)0)
                     W(flags) |= W_TEXT;
                  W(code) = T_FONT;
                  break;
                  }
         ...
         case E_BITCRT:  		/* create/destroy a bitmap */
			...
			/* download a bitmap.  Needs fixing to handle compressed maps */
         ...
         case E_BITLOAD:
               if (cnt >=2) {
...
#ifdef DEBUG
                     dprintf(*)(stderr,"%s: Cant display color image\r\n");
#endif
							break;
							}

                  if (format & 0x80)	/* 8 bits */
                     W(bitmap) = bit_alloc(W(esc[0]),W(esc[1]),0,
                                 BIT_DEPTH(W(window)));
                  else
                  ...
                  if (W(bitmap) != (BITMAP *) 0) {
                     W(flags) |= W_TEXT;
                     W(snarf) = (char *) BIT_DATA(W(bitmap));
                     }
                  W(esc)[TEXT_COUNT] = 0;
                  W(code) = T_BITMAP;
                  }
...
WINDOW *win;
	...
	if (W(text.wide) > 0) {
		set_winsize(ACTIVE(to_fd),
				W(text.high)/FSIZE(high), 
				W(text.wide)/FSIZE(wide));
#ifdef DEBUG
		...
		set_winsize(W(to_fd), 
				BIT_HIGH(W(window))/FSIZE(high), 
				BIT_WIDE(W(window))/FSIZE(wide));
#ifdef DEBUG
