...
/* bit_blit (and friends) are defined as a macro so machines with special
 * graphics hardware can use mem_rop a a special case
 */

#define bit_blit(dest,dx,dy,width,height,func,source,sx,sy)  \
	mem_rop1(dest,dx,dy,width,height,func,source,sx,sy)

#define bit_point(d,x,y,f)	mem_point1(d,x,y,f) 
#define bit_line(d,x0,y0,x1,y1,f)	mem_line1(d,x0,y0,x1,y1,f) 

/* Macro to declare a static bitmap */

#ifdef MOVIE
#define bit_static(name,wide,high,data,n)	\
	BITMAP name = {(DATA *) data, &name, 0, 0, wide, high, _STATIC, 0}
#else
#define bit_static(name,wide,high,data,n)	\
...
#define SET_DIRTY(x)		((x)->primary->type |= _DIRTY)

#define DOFLUSH()	SEND_TIME()	/* flush the stuff out */
...
#define BLACK		(~WHITE)		/* color table index for black */
