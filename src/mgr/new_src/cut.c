 ...
 */
/*	$Header: /home/sau/mgr/nsrc/RCS/cut.c,v 1.3 91/03/01 11:05:43 sau Exp Locker: sau $
	$Source: /home/sau/mgr/nsrc/RCS/cut.c,v $
*/
static char	RCSid_[] = "$Source: /home/sau/mgr/nsrc/RCS/cut.c,v $$Revision: 1.3 $";
		...
		for(j=0;j<count;j++)
			for(i = FSIZE(start);i<FSIZE(start)+FSIZE(count);i++) {
				if (W(font)->glyph[i+MAXGLYPHS*j] && i >= ' ') {
					hcode=get_hash(W(font)->glyph[i+MAXGLYPHS*j],
									0,0,FSIZE(wide),FSIZE(high),0);
					enter(hcode,i,j);
					}
				}
      }
...
int x,y,w,h;				/* position of glyph in "map" */
   ...
   for(entry=table[code];entry;entry=entry->next) {
      bit_blit(check,32-w,0,w,h,BIT_SRC,
				W(font)->glyph[entry->value+ entry->type*MAXGLYPHS],0,0);
