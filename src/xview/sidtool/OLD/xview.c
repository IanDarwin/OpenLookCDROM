...
Notify_event_type type ;
{
  ...
  static int oldy = 0 ;
  int curx, cury ;
  int dx, dy, id, isascii, isdown ;

  id      = event_id(event) ;
  isascii = event_is_ascii(event) ;
  isdown  = event_is_down(event) ;
  curx    = event_x(event) ;
  cury    = event_y(event) ;
 ...
 */
  ...
  if (isascii || !autoplay && (id == MS_RIGHT || id == LOC_MOVE))
    {
      c = id ;
      if (!autoplay)
        {
...
Canvas c ;
{
  ...
  if ((nfont = XLoadQueryFont(dpy, NFONT)) == NULL)
    {
      FPRINTF(stderr, "%s: can't open normal screen font.\n", progname) ;
      exit(1) ;
    }
  if ((bfont = XLoadQueryFont(dpy, BFONT)) == NULL)
    {
      FPRINTF(stderr, "%s: can't open bold screen font.\n", progname) ;
      exit(1) ;
    }
...
#ifdef WANTED
  XV_SET(c,  CANVAS_RETAINED, retained, 0) ;
#endif /*WANTED*/
}
