/* $Id: ps.c,v 1.20 92/07/03 08:06:30 pturner Exp Locker: pturner $
 *
...
double xconv(), yconv();
static FILE *psout;
static int psbb1, psbb2, psbb3, psbb4;
    ...
    int mode;
{
	...
	fprintf(psout, "%%%%BoundingBox: %d %d %d %d\n", psbb1 * 72 / 300, psbb2 * 72 / 300, psbb3 * 72 / 300, psbb4 * 72 / 300);
	fclose(psout);
	if (!ptofile) {
    ...
    int x2, y2, mode;
{
    ...
    if (mode) {
	fprintf(psout, "%d %d l\n", xtmp, ytmp);	/* lineto */
	psbb1 = min(xtmp, psbb1);
	psbb2 = min(ytmp, psbb2);
	psbb3 = max(xtmp, psbb3);
	psbb4 = max(ytmp, psbb4);
    } else {
...
char pscurfont[128] = "/Times-Roman findfont \n60 scalefont\n setfont";
int psfontsize = 60;
...
/*    fprintf(psout, "%%!PS-Adobe-2.0 EPSF-1.2\n");*/
    ...
    fprintf(psout, "%%%%BoundingBox: (atend)\n");
...
/*
 * rotate if in landscape mode, if other
 * parameters (PSXMAX etc.) are adjusted, this (i.e., the -2450) will
 * need adjusting also
 */
    if (dmode == 1) {
	fprintf(psout, "90 rotate\n");
	fprintf(psout, "0 -2450 translate\n");
    }
    pssetcolor(1);
    pssetlinewidth(1);
    setlinestyle(0);
