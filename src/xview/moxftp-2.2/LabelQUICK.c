/* $XConsortium: LabelQUICK.c,v 1.92 91/07/24 16:56:28 converse Exp $ */
...
/*
 * LabelQUICKQICK.c - LabelQUICK widget
 *
 */
...
#if defined(MOTIF)
#define SHADOW(lw)  \
	(lw->primitive.highlight_thickness + lw->primitive.shadow_thickness)
#endif
...
#define offset(field) XtOffsetOf(LabelQUICKRec, field)
static XtResource resources[] = {
    {XtNforeground, XtCForeground, XtRPixel, sizeof(Pixel),
	offset(label.foreground), XtRString, XtDefaultForeground},
#if defined(MOTIF)
    {XmNfontList, XmCFontList, XmRFontList, sizeof(XmFontList),
	offset(label.fontlist),XtRString, NULL},
#else
...
LabelQUICKClassRec labelQUICKClassRec = {
  {
/* core_class fields */	
    /* superclass	  	*/	(WidgetClass) superclass,
    /* class_name	  	*/	"LabelQUICK",
    /* widget_size	  	*/	sizeof(LabelQUICKRec),
...
#if defined(MOTIF)
  {                                    
#if defined(XmInheritBorderHighlight)
      XmInheritBorderHighlight,         
#else
      NULL,
#endif
#if defined(XmInheritBorderUnhighlight)
...
WidgetClass labelQUICKWidgetClass = (WidgetClass)&labelQUICKClassRec;
/****************************************************************
 *
 * Private Procedures
 *
...
static 
_XawLabelQUICKWidth16(fs, str, n)
    XFontStruct *fs;
    char *str;
    int	n;
{
...
static
_XawLabelQUICKDraw16(dpy, d, gc, x, y, str, n)
    Display *dpy;
    Drawable d;
    GC gc;
    int x, y;
...
#define XTextWidth16 _XawLabelQUICKWidth16
#define XDrawString16 _XawLabelQUICKDraw16
...
static void Initialize(request, new)
 Widget request, new;
{
    LabelQUICKWidget lw = (LabelQUICKWidget) new;

#if defined(MOTIF)
    if (lw->label.fontlist == NULL) {
	XmFontList fonttmp;

        fonttmp = _XmGetDefaultFontList( (Widget) lw, XmLABEL_FONTLIST);
        lw->label.fontlist = XmFontListCopy (fonttmp);
    }

    _XmFontListGetDefaultFont(lw->label.fontlist, &lw->label.font);

    if (request->core.width == 0) lw->core.width = 0;
    if (request->core.height == 0) lw->core.height = 0;
#endif
...
static void Redisplay(w, event, region)
    ...
    Region region;
{
   LabelQUICKWidget lw = (LabelQUICKWidget) w;
   GC gc;
...
static Boolean SetValues(current, request, new, args, num_args)
    ...
    Cardinal *num_args;
{
    LabelQUICKWidget curlw = (LabelQUICKWidget) current;
    LabelQUICKWidget reqlw = (LabelQUICKWidget) request;
    LabelQUICKWidget newlw = (LabelQUICKWidget) new;
    int i;
...
#if defined(MOTIF)
    if (curlw->label.fontlist != newlw->label.fontlist) {
	if (newlw->label.fontlist == NULL) {
            XmFontList fonttmp;

            fonttmp = _XmGetDefaultFontList((Widget) newlw, XmLABEL_FONTLIST);
            newlw->label.fontlist = XmFontListCopy (fonttmp);
        }
	newlw->label.fontlist = XmFontListCopy (newlw->label.fontlist);
        _XmFontListGetDefaultFont(newlw->label.fontlist, &newlw->label.font);
    } 
#endif
    ...
    newlw->label.update = was_resized || redisplay ||
...
{
    register LabelQUICKWidget lw = (LabelQUICKWidget)w;
...
char *new;
{
   ...
   if (lw->label.update) {
       if (old != lw->core.name) strcpy(lw->label.label, new);
       else		         lw->label.label = XtNewString(new);
       return;
   }

   if(!XtIsRealized(w)) {
        if (old == lw->core.name) {
        	lw->label.label = XtNewString(new);
	} else {
        	strcpy(lw->label.label, new);
	}
	return;
   }
   gc = XtIsSensitive((Widget)lw) ? lw->label.normal_GC : lw->label.gray_GC;
   ...
   if (lw->label.pixmap == None) {
	   ...
	   if (lw->label.encoding)
	       ...
	       x = lw->label.label_x;
	       for (i=0; i<len; i++) {
		   if (label[i] != old_label[i]) {
	           	XDrawImageString(XtDisplay(w), XtWindow(w), gc,
		                         x,  y, (char *)&label[i], 
			     	         1);
		   }
		   x += XTextWidth(lw->label.font, (char *)&label[i], 1);
	       }
           }
       }
   } else if (lw->label.label_len == 1) { /* depth */
