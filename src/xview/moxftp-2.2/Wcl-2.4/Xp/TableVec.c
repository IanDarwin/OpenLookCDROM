/*LINTLIBRARY*/
/*
 * SCCS_data:    @(#) TableVec.c	1.7 92/12/04 09:48:00
 */
...
XpTableVector  XpTableVectorNew(  size, tw, do_col )
    int			size;
    XpTableWidget	tw;
    int			do_col;
{
    ...
    if ( (XpTableLoc)0  == loc || 0 == size )
	return (XpTableVector)0;

    vec = (XpTableVector) XtCalloc( size+1, sizeof(XpTableVectorRec));
    ...
    */
    ...
    XpTableVectorMinimize(  vec, size, tw, do_col );
    return vec;
}

void XpTableVectorFree(  vec )
    XpTableVector  vec ;
{
    XtFree( (char*)vec );
}
...
*/
void XpTableVectorMinimize(  vec, vec_len, tw, do_col )
    XpTableVector	vec;
    int			vec_len;
    XpTableWidget	tw;
    int			do_col;
{
    ...
    if ( (XpTableVector)0  == vec || 0 == vec_len )
	return;
    ...
    */
	...
	qsort( (char*)loc,			/* sort real_layout	*/
		XpTableLocLen(loc),
		sizeof(XpTableLocRec),
		XpTableLocCompareColSpan );	/* compare column spans	*/
    else
	qsort( (char*)loc,			/* sort real_layout	*/
		XpTableLocLen(loc),
		sizeof(XpTableLocRec),
		XpTableLocCompareRowSpan );	/* compare row spans	*/
	...
	*/
	...
	{
	    pref = XpTableLocPreferredWidth(  loc, tw );
	...
	{
	    pref = XpTableLocPreferredHeight(  loc, tw );
...
static int XpTableVectorSize(  vec, num, tw, do_col, do_actual )
    XpTableVector	vec;
    int			num;
    XpTableWidget	tw;
    int			do_col;
    int			do_actual;
{
	...
	size  = 2*tw->table.margin_width;
    }
    else
    {
	space = tw->table.row_spacing;
	size  = 2*tw->table.margin_height;
    }

    if ( 0 != num && (XpTableVector)0  != vec )
    {
	if (do_actual)
	{
...
int XpTableVectorTotalSize(  vec, num, tw, do_col )
    XpTableVector	vec;
    int			num;
    XpTableWidget	tw;
    int			do_col;
{
    return XpTableVectorSize(  vec, num, tw, do_col, DO_ACTUAL );
}

int XpTableVectorPreferredSize(  vec, num, tw, do_col )
    XpTableVector	vec;
    int			num;
    XpTableWidget	tw;
    int			do_col;
{
    return XpTableVectorSize(  vec, num, tw, do_col, DO_PREFERRED );
}
...
void XpTableVectorAdjust(  vec, vec_len, change )
    XpTableVector  vec;
    int            vec_len, change;
{
    ...
    if ( (XpTableVector)0  == vec || 0 == vec_len || 0 == change )
	return;
...
void XpTableVectorComputeOffsets(  vec, vec_len, margin, gap )
    XpTableVector  vec;
