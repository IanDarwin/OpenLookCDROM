...
#ifndef lint
static char	sccsid[] = "@(#)gcm.c	1.22 90/06/01 Copyright 1989 Sun Microsystems";
#endif
	...
	char           *fg;			/* initial fg | NULL */
{
	...
	/*	new_bg = default_bg_index(win, cms);  - paw */
		new_bg = default_bg_index(win);

	if (!fg || ((new_fg = gcm_color_index(fg)) == -1))
	/*	new_fg = default_fg_index(win, cms);  paw */
...
/*	static int	strcasecmp(); */
	int	strcasecmp(); /* changed 9/13 due to paw report from ANSI C compiler */
