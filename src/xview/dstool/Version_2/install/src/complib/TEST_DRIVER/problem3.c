...
int			method, data_type;
{
  ...
  int  			prob3_varb_watch_func();

  static int
      prob3_fstop_indices_array[]={0},
      prob3_cross_orient_array[]={PM_and_MP},
      prob3_cross_mode_array[]={SOLVE_STEP}; 

  static double
      prob3_target_values_array[]={0.5}, 
      prob3_target_tols_array[]={0.001};

  static fstop_group
      prob3_var_fstop_group_values = 
  { prob3_varb_watch_func, 1, 
	prob3_fstop_indices_array, prob3_cross_orient_array, prob3_cross_mode_array,
	prob3_target_values_array, prob3_target_tols_array }; 


  if( data_type == VALUES )
     {
      prop_cntl->function		= pb3_fp;
  ...
  else
     {
      ...
      prop_cntl->fstop_grp_list = (fstop_group **) calloc(prop_cntl->num_fstop_grps,
							sizeof(fstop_group *));
      prop_cntl->fstop_grp_list[0] = &prob3_var_fstop_group_values;
...
/* vector field for test problem 3 */
pb3_fp(f, x, p)
double f[],x[],p[];
{
  double	t = x[3];
...
/* exact solution to test problem 3 */
int
exact_pb3( y, t, p )
double  t, *y, *p;
{
...
int
    prob3_varb_watch_func(x,p,index,result)
int	
    index;
double  
    *result,x[],p[];
