...
int			method, data_type;
{
  int			i, j;
  extern int		pb4_fp();
  static double		state[]={3.0,0.0,0.0,0.0}, 
			parameters[]={1.0},
  			min[]=NULL,
			max[]=NULL,
			period_start[]=NULL,
  ...
  int  			prob4_varb_watch_func();

  static int
      prob4_fstop_indices_array[]={0},
      prob4_cross_orient_array[]={PM_and_MP},
      prob4_cross_mode_array[]={SOLVE_STEP}; 

  static double
      prob4_target_values_array[]={0.5}, 
      prob4_target_tols_array[]={0.001};

  static fstop_group
      prob4_var_fstop_group_values = 
  { prob4_varb_watch_func, 1, 
	prob4_fstop_indices_array, prob4_cross_orient_array, prob4_cross_mode_array,
	prob4_target_values_array, prob4_target_tols_array }; 


  if( data_type == VALUES )
     {
      prop_cntl->function		= pb4_fp;
  ...
  else
     {
      ...
      prop_cntl->fstop_grp_list = (fstop_group **) calloc(prop_cntl->num_fstop_grps,
							sizeof(fstop_group *));
      prop_cntl->fstop_grp_list[0] = &prob4_var_fstop_group_values;
...
/* vector field for test problem 4 */
pb4_fp(f, x, p)
double f[],x[],p[];
{
  double	t = x[2];
  ...
  f[0] = x[1];
  f[1] = p[0]*(1.0 - x[0]*x[0])*x[1] - x[0];

  return(0);
}


/* exact solution to test problem 4 */
int
exact_pb4( y, t, p )
double  t, *y, *p;
{
...
int
    prob4_varb_watch_func(x,p,index,result)
int	
    index;
double  
    *result,x[],p[];
