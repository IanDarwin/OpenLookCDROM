...
/* ----------------------------------------------------------------------------------------------------------------
   problem 2:  This is a two-dimensional example, constructed from test problems (A2) and (A4) published in
	    
		 Hull, T.E., et al., `Comparing Numerical Methods for Ordinary Differential Equations,'
		 SIAM Journal of Numerical Analysis, Vol. 9, No. 4, 1972, pps. 603-637.

			y' = -y^3/2              IC:   y(0) = 1
			z' = (z/4)*(1-z/20)            z(0) = 1

			solution: y(t) = 1/sqrt(t+C) ,  C = 1
				  z(t) = 20/( 1 + 19Ce^(-t/4) )  

               The stopping critera used is for a fixed number of steps (200).  The primary purpose of 
	       integrating this (uncoupled) system is to excercise the propagation of a non-linear system
	       of ODE's with known solution.
...
int			method, data_type;
{
  int			i, j;
  extern int		pb2_fp();
  static double		state[]={1.0,1.0,0.0}, 
			parameters[]={1.0},
  			min[]=NULL,
			max[]=NULL,
			period_start[]=NULL,
...
double  x, *y, *p;
{
  ...
  y[0] = 1.0/sqrt( x+p[0] );
  y[1] = 20.0/( 1.0 + 19.0*p[0]*exp(-x/4.0) );  
  return(0);
}
