...
int			method, data_type;
{
  ...
  int  			prob1_varb_watch_func();

  static int
      prob1_fstop_indices_array[]={0},
      prob1_cross_orient_array[]={PM_and_MP},
      prob1_cross_mode_array[]={SOLVE_STEP}; 

  static double
      prob1_target_values_array[]={0.5}, 
      prob1_target_tols_array[]={0.001};

  static fstop_group
      prob1_var_fstop_group_values = 
  { prob1_varb_watch_func, 1, 
	prob1_fstop_indices_array, prob1_cross_orient_array, prob1_cross_mode_array,
	prob1_target_values_array, prob1_target_tols_array }; 


  if( data_type == VALUES )
     {
      prop_cntl->function		= pb1_fp;
  ...
  else
     {
      ...
      prop_cntl->fstop_grp_list = (fstop_group **) calloc(prop_cntl->num_fstop_grps,
							sizeof(fstop_group *));
      prop_cntl->fstop_grp_list[0] = &prob1_var_fstop_group_values;
...
/* vector field for test problem 1 */
pb1_fp(f, x, p)
double f[],x[],p[];
{
  f[0] = (x[0]/4.0)*(p[0] - x[0]/20.0); 
  return(0);
}


/* exact solution to test problem 1 */
int
exact_pb1( y, t, p )
double  t, *y, *p;
{
  ...
  y[0] = 20.0/( 1.0 + 19.0*exp(-t/4.0) );
  return(0);
}

/* substitute for varb_watch_func in compute_orbit.c */
int
    prob1_varb_watch_func(x,p,index,result)
int	
    index;
double  
    *result,x[],p[];
