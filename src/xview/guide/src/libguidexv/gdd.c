...
#ifndef lint
static char	sccsid[] = "@(#)gdd.c	2.44 92/08/17 Copyright 1991 Sun Microsystems";
#endif
...
GDD_DROP_SITE			*Current_drag_site;
...
init_dnd_code_table()
{
	Dnd_codes[0] = (char *) dgettext("libguidexv",
					 "Dragdrop: Completed");
	Dnd_codes[1] = (char *) dgettext("libguidexv",
					 "Dragdrop: Error");
	Dnd_codes[2] = (char *) dgettext("libguidexv",
					 "Dragdrop: Illegal Target");
	Dnd_codes[3] = (char *) dgettext("libguidexv",
					 "Dragdrop: Timed out");
	Dnd_codes[4] = (char *) dgettext("libguidexv",
					 "Dragdrop: Unable to obtain selection");
	Dnd_codes[5] = (char *) dgettext("libguidexv",
					 "Dragdrop: Dropped on root window");
	Dnd_codes[6] = (char *) dgettext("libguidexv",
					 "Dragdrop: Aborted");
	Dnd_codes[7] = (char *) dgettext("libguidexv",
	...
	else if (len == SEL_ERROR)
	{
		...
		case SEL_BAD_PROPERTY :
      		...
      		case SEL_BAD_WIN_ID:
          		xv_set(Base_frame, FRAME_LEFT_FOOTER,
			       dgettext("libguidexv",
					"Dragdrop: Bad window id"), 0);
          		break;
      		case SEL_TIMEDOUT:
          		xv_set(Base_frame, FRAME_LEFT_FOOTER,
			       dgettext("libguidexv",
					"Dragdrop: Timed out"), 0);
          		break;
      		case SEL_PROPERTY_DELETED:
          		xv_set(Base_frame, FRAME_LEFT_FOOTER,
			       dgettext("libguidexv",
					"Dragdrop: Property deleted"), 0);
          		break;
      		case SEL_BAD_PROPERTY_EVENT:
          		xv_set(Base_frame, FRAME_LEFT_FOOTER,
			       dgettext("libguidexv",
					"Dragdrop: Bad property event"), 0);
          		break;
		}
		stop_dragdrop();
		return;
...
#ifdef linux
				D->source_name[len] = (char)NULL;
#else
				D->source_name[len] = NULL;
#endif
				if (!D->processing_stream)
					D->state_mask |= HOST_RESPONSE;
			}
...
#ifdef linux
				D->source_name[old_length + len] = (char)NULL;
#else
				D->source_name[old_length + len] = NULL;
#endif
			}
		}
		else
...
#ifdef linux
				D->source_host[old_length + len] = (char)NULL;
#else
...
#ifdef linux
				D->source_filename[len] = (char)NULL;
#else
...
#ifdef linux
				D->source_filename[old_length + len] = (char)NULL;
#else
...
#ifdef linux
				D->data_label[old_length + len] = (char)NULL;
#else
	...
	if (D->state_mask == RESPONSE_LEVEL_1)
	{
		DP(("first batch of replies processed, asking for second\n"));
                ...
                if ((!atom_in_list(XA_STRING, D->target_list, D->num_targets)) &&
                    (!atom_in_list(atm_file_name_atom, D->transport_list, D->num_transports)) &&
                    (!atom_in_list(text_atom, D->target_list, D->num_targets)))

		{
			xv_set(Base_frame, FRAME_LEFT_FOOTER, dgettext("libguidexv","Dragdrop: Unknown type of data"), 0);
			stop_dragdrop();
			return;
		}
	...
	*/
	...
	if ((*type == selection_end_atom) || (*type == dragdrop_done_atom))
	{
		 ...
		 */
        	...
        	Current_drag_site->drag_func(Current_drag_site->item, 
				     Current_drag_site->event, 
				     NULL, 
				     GDD_DRAG_COMPLETED);	

		xv_set(xv_get(Current_drag_site->item, PANEL_DROP_DND), 
			SEL_CONVERT_PROC, NULL, 
			NULL);

		xv_set(Base_frame, 
			FRAME_LEFT_FOOTER, dgettext("libguidexv", "Dragdrop: Completed"), 
			NULL);
	...
	{
		if (Drop_info->app_name && *Drop_info->app_name)
		{
			*format = 8;
...
#ifdef linux
		xfer_data_buf[*length] = (char)NULL;
#else
...

{
	 ...
	 * This code can be used to support a move operation.
	  ...
	  {
	  xv_set(Sel, SEL_TYPE, delete_atom, 0);
...
#endif
{
	...
	xv_set(Sel, SEL_TYPE, enumeration_count_atom, 0);
...
#endif
{
	...
	case ACTION_DRAG_MOVE:
		...
		if (!Sel)
			Sel = xv_create(window, SELECTION_REQUESTOR, 0);
...
#endif
{
	...
	case LOC_DRAG:
		if (value == XV_OK)
		{
			Current_drag_site = NULL;

			for (ds = Drop_sites; ds; ds = ds->next)
			{
				if (!ds->is_drop_site && item == ds->drop_site)
				{
					Current_drag_site = ds;
					Current_drag_site->item = item; 
					Current_drag_site->event = event; 
					break;

				}
			}

			if (Current_drag_site && Current_drag_site->drag_func)
			{
				Drop_info = (GDD_DROP_INFO *) calloc(1, sizeof(GDD_DROP_INFO));
				Current_drag_site->drag_func(item, event, Drop_info, GDD_DRAG_STARTED);	
...
#endif
{
	...
	current_type = (Atom)NULL;
	
	D = (GDD_CONTEXT *) malloc(sizeof(GDD_CONTEXT));
#ifdef linux
	memset((char *) D, (int)NULL, sizeof(GDD_CONTEXT));
#else
	memset((char *) D, NULL, sizeof(GDD_CONTEXT));
#endif
...
#endif
{
	...
	if (rectlist)
	{
		/* How many rects passed in? */
		for (count = 0, rp = rectlist; !rect_isnull(rp); rp++, count++)
			;

		if (!ds->rectlist)
			ds->rectlist = (Rect *)calloc((count + 1), rectsize);
		else 
		{
			/* How big is ds->rectlist now? */
			for (i = 0; !rect_isnull(&ds->rectlist[i]); i++)
				;

			/* If not same size, allocate new array */
			if (i != count)
			{
				free(ds->rectlist);
				ds->rectlist = (Rect *) calloc((count + 1), rectsize);
		...
		/* Copy passed in rects into local structure */
	...
	{
		free(ds->rectlist);
		xv_set(ds->drop_site, DROP_SITE_DELETE_REGION_PTR, NULL, 0);
	}
}
...
#endif
{
	if (di && di->data && *di->data && !di->length)
