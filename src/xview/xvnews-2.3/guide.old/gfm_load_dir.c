...
#ifndef lint
static char	sccsid[] = "@(#)gfm_load_dir.c	2.17 91/08/14 Copyright 1990 Sun Microsystems";
#endif
...
gfm_initialize_glyphs()
{
	...
	Gfm_folder_glyph = (Server_image) xv_create((Xv_opaque)NULL, SERVER_IMAGE,
		XV_WIDTH, 	16,
		XV_HEIGHT, 	16,
		SERVER_IMAGE_BITS, 	folder_bits,
		NULL);

	Gfm_dotdot_glyph = (Server_image) xv_create((Xv_opaque)NULL, SERVER_IMAGE,
		XV_WIDTH, 	16,
		XV_HEIGHT, 	16,
		SERVER_IMAGE_BITS, 	dotdot_bits,
		NULL);

	Gfm_executable_glyph = (Server_image) xv_create((Xv_opaque)NULL, SERVER_IMAGE,
		XV_WIDTH, 	16,
		XV_HEIGHT, 	16,
		SERVER_IMAGE_BITS, 	executable_bits,
		NULL);

	Gfm_document_glyph = (Server_image) xv_create((Xv_opaque)NULL, SERVER_IMAGE,
		XV_WIDTH, 	16,
		XV_HEIGHT, 	16,
		SERVER_IMAGE_BITS, 	document_bits,
		NULL);

	Gfm_broken_link_glyph = (Server_image) xv_create((Xv_opaque)NULL, SERVER_IMAGE,
		XV_WIDTH, 	16,
		XV_HEIGHT, 	16,
		SERVER_IMAGE_BITS, 	broken_link_bits,
		NULL);

	Gfm_system_doc_glyph = (Server_image) xv_create((Xv_opaque)NULL, SERVER_IMAGE,
		XV_WIDTH, 	16,
		XV_HEIGHT, 	16,
	...
	int			flg;
{
	...
	/*
	 * Set current directory and open for reading
	 */
	...
	GFM_TYPE	  type;
{
	if (Gfm_list_count == Gfm_list_lth) {
		if (Gfm_list)
			Gfm_list = (GFM_LIST_ENTRY *) realloc(Gfm_list,
					(Gfm_list_lth += GFM_LIST_INC) *
					sizeof (GFM_LIST_ENTRY));
		else
			Gfm_list = (GFM_LIST_ENTRY *) malloc(
			    (Gfm_list_lth += GFM_LIST_INC) * sizeof (GFM_LIST_ENTRY));
	}
	...
	case GFM_SYSDOC:
		...
		break;
	default:
		Gfm_list[Gfm_list_count].glyph = (Xv_opaque)NULL;
	...
	GFM_PRIVATE		*gfm_private;
{
	int		width;
	int		nrows;
	int		rows = xv_get(ip->list, PANEL_LIST_NROWS);
	Xv_opaque	old_list;

	xv_set(ip->list, PANEL_LIST_DELETE_ROWS, 0, rows, NULL);
	/*
