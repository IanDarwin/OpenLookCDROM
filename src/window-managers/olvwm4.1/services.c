...
AppMenuFunc(dpy, winInfo, menuInfo, idx)
	...
	int     	idx;
{
	return ExecCommand(winInfo->core.client->scrInfo->environment,
	...
	int     	idx;
{
	...
	else if ( pid == 0 )
	{
		...
		fprintf( stderr, GetString("olvwm: psh error: %d\n"), errno );
	}
	else
	{
	...
	int     	idx;
{
	...
	UseNoticeBox(dpy, winInfo->core.client->screen, &noticeBox);
	FreeText(buttons[0]);
	FreeText(msg);
}
...
PropertiesFunc(dpy, winInfo, menuInfo, idx)
	...
	int     	idx;
{
	return ExecCommand(winInfo->core.client->scrInfo->environment,WORKSPACEPROPS);
}
...
ReReadUserMenuFunc(dpy, winInfo, menuInfo, idx)
	...
	int     	idx;
{
	ReInitUserMenu(dpy,winInfo->core.client->scrInfo,True);
}
...
/*ARGSUSED*/
int
WindowStickAction(dpy, winInfo, menuInfo, idx)
	Display		*dpy;
	WinGeneric	*winInfo;
	MenuInfo	*menuInfo;
	int		idx;
{
	ClientStickUnstickToggle(winInfo->core.client);
}
...
/*
 * _dismissSiblingMenus - called from VisitPinnedMenuclients to
...
/*
 * StickSelnFunc
 *	Toggles the sticky bit on all selected clients
 */
/*ARGSUSED*/
int
StickSelnFunc(dpy, winInfo, menuInfo, idx)
	Display 	*dpy;
	WinGeneric 	*winInfo;
	MenuInfo    	*menuInfo;
	int     	idx;
