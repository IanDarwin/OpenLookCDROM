#ifndef lint
static char rcsid[] =
    "@(#) $Header: rcons_subr.c,v 1.1 93/09/03 23:27:38 jef Locked $ (LBL)";
#endif
...
void rcons_cursor();
void rcons_invert();
void rcons_clear2eop();
void rcons_clear2eol();
void rcons_scroll();
void rcons_delchar();
void rcons_delline();
void rcons_insertchar();
void rcons_insertline();

extern void rcons_bell();
	...
	register struct fbdevice *fb;
{
	...
	if (fb->fb_cursor)
		raster_op(fb->fb_sp, x, y,
		    fb->fb_cursor->width, fb->fb_cursor->height,
		    RAS_XOR, fb->fb_cursor, 0, 0);
	else
#endif
		raster_op(fb->fb_sp, x, y,
#ifdef notdef
