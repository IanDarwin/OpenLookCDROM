#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_filter.c 20.48 93/06/28";
#endif
#endif
 ...
 */
{
    ...
    textsw_init_selection_object(folio, fill_in, _xv_null_string_wc, 0, FALSE);
#else
    ...
    Es_index       *delta;
{
	...
	switch ((Textsw_filter_command) (int_buffer[1])) {
	  ...
	  case TEXTSW_FILTER_SET_SELECTION:
		...
		goto Deal_With_Slop;
#ifdef OW_I18N
	    textsw_set_selection_wcs(VIEW_REP_TO_ABS(folio->first_view),
#else
	    textsw_set_selection(VIEW_REP_TO_ABS(folio->first_view),
#endif
