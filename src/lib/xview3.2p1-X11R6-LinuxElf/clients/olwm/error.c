    ...
    XErrorEvent *event;
{
	...
	printError(dpy, event,
	    GetString("olwm: warning, X extension error -- "));
    }
}
	...
	XErrorEvent *event;
{
	...
	if (errBitmask & entry->fatal) {
		...
		if (GRV.PrintWarnings) {
		    printError(dpy, event,
			GetString("olwm: warning, X protocol error -- "));
		}
	}
...
ErrorGeneral(txt)
	char *txt;
{
        FPRINTF(stderr, GetString("olwm: fatal error -- %s\n"), txt);
#ifdef DEBUG
	abort();
	/*NOTREACHED*/
#else
...
{
	if (GRV.PrintWarnings)
		FPRINTF(stderr, GetString("olwm: warning -- %s\n"), txt);
}
