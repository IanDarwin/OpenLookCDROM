...
Client *cli;
{
	...
	/* Remove this client from the global list of clients. */
...
#ifdef DEBUG
	/* fill with identifiable trash */
	memset((char *) cli, 0xbd, sizeof(Client));
#endif /* DEBUG */
...
/*
 * ClientSetWMState     -- sets the WMState field of the clietn and
...
{
    VisitPinnedMenuClients(DestroyClient, NULL);
}
    ...
    Time time;
{
    ...
    /*
     * QueryTree returns children in bottom-to-top order, so search backward.
     */
