...
Vector *tr_vec;
int vec_xinitpos, vec_yinitpos;
int vec_xdiff, vec_ydiff;

void
trvec_proc(Menu *m, Menu_item *mi)
{
	float fx, fy;
	int ix, iy;
	...
	if ((tr_vec= (Vector *) menuitem_get(mi, LXMI_CLIENTDATA)) == (Vector *) NULL) {
		ice_err("Cannot locate selected vector object.", NONFATAL);
		ice_op= MAIN_MENU;
		return;
	}

	ice_op= VEC_TRANSLATE;
...
vectr_event(XEvent *event)
{
	...
	case ButtonRelease:
		...
		XSync(dpy, False);
		tr_vec->setploc(x, pg_pixheight-1-y, (float) pg_dpi);
		tr_vec->getend(&fx, &fy, &oex, &oey);
		ex= oex+(x-vec_xinitpos);
		ey= oey+(pg_pixheight-1-y-vec_yinitpos);
		tr_vec->setpend(ex, ey, (float) pg_dpi);
		pg_draw();
	...
	vec_x= x;
	vec_y= y;
	vec_ex= vec_x+vec_xdiff;
	vec_ey= vec_y+vec_ydiff;
	XDrawLine(dpy, pg_cpm, pg_xgc, vec_x, vec_y, vec_ex, vec_ey);
	XDrawLine(dpy, pg_cwin, pg_xgc, vec_x-vx, vec_y-vy, vec_ex-vx, vec_ey-vy);
	vec_drawn= TRUE;
