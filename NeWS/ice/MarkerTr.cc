...
void
trmrk_proc(Menu *m, Menu_item *mi)
{
	float fx, fy;
	int ix, iy;
	char xbuf[30], ybuf[30];

	if ((tr_mrk= (Marker *) menuitem_get(mi, LXMI_CLIENTDATA)) == (Marker *) NULL) {
		ice_err("Cannot locate selected marker object.", NONFATAL);
		ice_op= MAIN_MENU;
		return;
	}

	ice_op= MRK_TRANSLATE;
...
mrktr_event(XEvent *event)
{
	...
	case ButtonRelease:
		...
		x= bevt->x+vx;
		y= bevt->y+vy;
		tr_mrk->setploc(x, pg_pixheight-1-y, (float) pg_dpi);
		pg_draw();
