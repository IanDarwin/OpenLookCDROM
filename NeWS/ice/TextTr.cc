...
void
trtext_proc(Menu *m, Menu_item *mi)
{
	float fx, fy;
	int ix, iy;
	char xbuf[30], ybuf[30];

	if ((tr_text= (Text *) menuitem_get(mi, LXMI_CLIENTDATA)) == (Text *) NULL) {
		ice_err("Cannot locate selected text object.", NONFATAL);
		ice_op= MAIN_MENU;
		return;
	}

	if (tr_text->getjustify() == TEXT_PATH)
		ice_err("Warning -- translation of path-mapped text is meaningless.", NONFATAL);

	ice_op= TEXT_TRANSLATE;
...
texttr_event(XEvent *event)
{
	...
	case ButtonRelease:
		...
		x= bevt->x+vx;
		y= bevt->y+vy;
		tr_text->setploc(x, pg_pixheight-1-y, (float) pg_dpi);
		pg_draw();
